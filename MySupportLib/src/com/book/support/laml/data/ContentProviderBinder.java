...
import com.book.support.laml.elements.ImageScreenElement;
import com.book.support.laml.elements.ListScreenElement;
import com.book.support.laml.elements.ScreenElement;
import com.book.support.laml.elements.ListScreenElement.ColumnInfo;
import com.book.support.laml.elements.ListScreenElement.DataWrapper;
import com.book.support.laml.elements.ListScreenElement.DeleteExtra;
import com.book.support.laml.util.IndexedNumberVariable;
import com.book.support.laml.util.TextFormatter;
import com.book.support.laml.util.Utils;
import com.book.support.util.LocationUtil;
...
//                        mRoot.requestUpdate();
    ...
    private void registerObserver(Cursor c, boolean reg) {
        if (c != null) {
            if (reg) {
                if (mNotifyUri != null) c.setNotificationUri(getContext().mContext.getContentResolver(),
    ...
    private boolean updateVariables() {
        synchronized (mCursorLock) {
            ...
            if (mCursor == null || (count = mCursor.getCount()) == 0) {
                ...
                for (com.book.support.laml.data.VariableBinder.Variable v : mVariables) {
                ...
                for (com.book.support.laml.data.VariableBinder.Variable variable : mVariables) {
    ...
    private boolean startQueryData(){
        ...
        else onQueryComplete(getContext().mContext.getContentResolver().query(uri, mColumns, where, mArgs, mOrder));
...
//                }
    ...
    private static class List {
        ...
        public List(Element node, ScreenElementRoot root){
            mRoot = root;
            mName = node.getAttribute("name");
            mContentResolver = root.getContext().mContext.getContentResolver();
                        ...
                        if (replaceIndex != -1 && position == replaceIndex && objects[position] instanceof String) {
                            ...
                            if(isContacts(phoneNumber)){
                                objects[position] =getReplaceData(phoneNumber, mReplaceUri, mReplaceColumn);
                                   ...
                                   String yellowName =getYellowPageNumberName(null, phoneNumber);
