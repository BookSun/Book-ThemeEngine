...
import com.book.support.laml.SoundManager.SoundOptions;
import com.book.support.laml.data.DateTimeVariableUpdater;
import com.book.support.laml.data.VariableBinder;
import com.book.support.laml.data.VariableBinderManager;
import com.book.support.laml.data.VariableNames;
import com.book.support.laml.data.VariableUpdaterManager;
import com.book.support.laml.data.Variables;
import com.book.support.laml.elements.ButtonScreenElement;
import com.book.support.laml.elements.ElementGroup;
import com.book.support.laml.elements.FramerateController;
import com.book.support.laml.elements.ITicker;
import com.book.support.laml.elements.ScreenElement;
import com.book.support.laml.elements.ButtonScreenElement.ButtonAction;
import com.book.support.laml.util.ConfigFile;
import com.book.support.laml.util.IndexedNumberVariable;
import com.book.support.laml.util.Task;
import com.book.support.laml.util.Utils;
import com.book.support.laml.util.ConfigFile.Variable;
...
public class ScreenElementRoot extends ScreenElement implements InteractiveListener {
    ...
    private float DEFAULT_FRAME_RATE = 30;
    ...
    public ScreenElementRoot(ScreenContext c) {
        ...
        mSoundManager = new SoundManager(mContext.mContext, mContext.mResourceManager);
    ...
    public void init() {
        ...
        LanguageHelper.load(mContext.mContext.getResources().getConfiguration().locale,
        ...
        initCamera(mContext.mContext);
    }
    
    private void initCamera(Context context) {
        Intent intent = new Intent("book.media.action.STILL_IMAGE_CAMERA");
        PackageManager pm = context.getPackageManager();
        ResolveInfo info = pm.resolveActivity(intent, 0);
        if (info != null && info.activityInfo != null && info.activityInfo.packageName.contains("book")
    ...
    private void setupScale(Element root) {
        ...
        Display display = ((WindowManager) mContext.mContext.getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay();
        ...
        if (mDefaultScreenWidth != 0 && mDefaultResourceDensity != 0) {
            if (mScaleByDensity) {
                ...
                if ("Icon".equals(mName)) {
                    mScale *= com.book.support.util.Constants.ICON_SCALE;
                }
            } else {
                mScale = (float) mScreenWidth / (float) mDefaultScreenWidth;
                if ("Icon".equals(mName)) {
                    // use mRawIconScale instead of mIconScale because 480 / 720 * 128 = 85.
                    mScale = com.book.support.util.Constants.CUSTOM_ICON_WIDTH / Float.parseFloat(FANCY_SIZE+"");
                }
