...
import book.support.v7.internal.view.menu.MenuWrapperFactory;
import book.support.v7.internal.widget.ActionBarContainer;
import book.support.v7.internal.widget.ActionBarContextView;
import book.support.v7.internal.widget.ActionBarView;
import book.support.v7.internal.widget.ProgressBarICS;
...
//    };
    ...
    private bookActionBarContainer splitActionBarView;
		...
		if (!IS_IGNORE_HOME) {
			try {
				...
				if (mActivityManager != null) {
					...
					// .equals("com.book.launcher.Launcher");
...
//                return;
//            }
//            actionMenus = st.menu.bookGetVisibleItems();
        	if (mMenu != null) {
        		 actionMenus = mMenu.bookGetVisibleItems();
        	}
        } else {
        	if (mActionMode != null) {
        		 MenuBuilder menu = (MenuBuilder) mActionMode.getMenu();
                 if (menu != null) {
                     actionMenus = menu.bookGetVisibleItems();
                 }
        	}
        ...
        for (int i = 0; i < actionMenus.size(); i++) {
            ...
            if (item.showsTextAsAction()) {
                menuStyle = ActionBar.book_ACTION_MENU_STYLE_ICON;
                actionFlag ^= MenuItem.SHOW_AS_ACTION_WITH_TEXT;
                item.setShowAsAction(actionFlag);
            } else {
                menuStyle = ActionBar.book_ACTION_MENU_STYLE_ICON_WITH_TEXT;
                actionFlag |= MenuItem.SHOW_AS_ACTION_WITH_TEXT;
                item.setShowAsAction(actionFlag);
            }
      ...
      void saveActionMenuStyle(int menuStyle) {
         ...
         Settings.System.putInt(mActivity.getContentResolver(), ActionBar.book_ACTION_MENU_STYLE, menuStyle);
     }
    @Override
...
/*            *//**
            ...
            splitActionBarView = (bookActionBarContainer)mActivity.findViewById(R.id.split_action_bar);
            if (splitActionBarView != null) {
            	
            	splitActionBarView.setOnActionMenuDoubleClickListener(new bookActionBarContainer.OnActionMenuDoubleClickListener() {
            	...
            	splitActionBarView.setOnActionModeMenuDoubleClickListener(new bookActionBarContainer.OnActionMenuDoubleClickListener() {
            	...
            	splitActionBarView.setOnActionOptionMenuSlideListener(
                         new bookActionBarContainer.OnActionSlideListener() {
...
//                        .getBoolean(R.styleable.Theme_windowActionBar, false);
    ...
    public boolean onMenuItemSelected(int featureId, MenuItem item) {
        ...
        if(mActionBarView != null && mActionBarView.getSplitView() != null ){
            mSplitView = (bookActionBarContainer)(mActionBarView.getSplitView());
...
//                    R.style.Theme_AppCompat_CompactMenu);
    ...
    protected bookActionBarContainer mSplitView;
    @Override
    public boolean onBackPressed() {
        ...
        if (mActionMode != null) {
        	if(mActionModeView != null && mActionModeView.mSplitView != null ){
        		...
        		mSplitView = (bookActionBarContainer)(mActionModeView.mSplitView);
...
//						} 
    ...
    void updateActionMenuCount(ActionBarContainer actionMenuView, Menu menu) {
        ...
        if (actionMenuView != null) {
            if (actionMenuView instanceof bookActionBarContainer) {
                ...
                bookActionBarContainer.setNonActionItemsSize(itemSize);

                itemSize = menuBuilder.bookGetActionItems().size();
                bookActionBarContainer.setActionItemsSize(itemSize);
            }
        }
    }
