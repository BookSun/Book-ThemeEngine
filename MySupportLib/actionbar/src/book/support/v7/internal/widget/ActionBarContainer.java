...
// book ADD END
    ...
    static class Injector {
        static void initBgResources(Context context, AttributeSet attrs, ActionBarContainer abc) {
            //try {
            //	abc.mColorfulMode = System.getInt(context.getContentResolver(), Settings.book_COLORVIEW_MODE);
            //	android.content.res.TypedArray a = context.obtainStyledAttributes(attrs,
            //       com.book.internal.R.styleable.EnableColorfulView);
            //	abc.mIsEnableColorful = a.getBoolean(
            //		com.book.internal.R.styleable.EnableColorfulView_enable_colorful_actionbar_background, true);
            //} catch (Exception e) {
            //	e.printStackTrace();
            //}

            abc.mIsActionBarOverlay = true;//bookUiUtil.isActionBarOverlay(context);

            //if (!abc.mIsbookUi || abc.mColorfulMode == Settings.book_COLORVIEW_MODE_INVALID || !abc.mIsEnableColorful || abc.mIsActionBarOverlay) {
            //	return;
            //}

            //if (abc.mColorfulMode == Settings.book_COLORVIEW_MODE_WALLPAPER) {
            //	getWallPaperBg(context, abc);
            //} else if (abc.mColorfulMode == Settings.book_COLORVIEW_MODE_GAUSSIAN) {
            //	getGaussianBg(context, abc);
            //} else if (abc.mColorfulMode == Settings.book_COLORVIEW_MODE_NONE) {
