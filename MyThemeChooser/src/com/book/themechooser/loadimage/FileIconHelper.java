package com.book.themechooser.loadimage;
...
import com.book.themechooser.loadimage.FileCategoryHelper.FileCategory;
import com.book.themechooser.loadimage.FileIconLoader.IconLoadFinishListener;
...
public class FileIconHelper implements IconLoadFinishListener {
    ...
    private static void addItem(String[] exts, int resId) {
        if (exts != null) {
            for (String ext : exts) {
                fileExtToIcons.put(ext.toLowerCase(Locale.US), resId);
            }
        }
    }

    public static int getFileIcon(String ext) {
        Integer i = fileExtToIcons.get(ext.toLowerCase(Locale.US));
        if (i != null) {
            return i.intValue();
        } else {
            // modification 1
    ...
    public void setIcon(ThumbnailBase fileInfo, ImageView fileImage,
                        ImageView fileImageFrame, int picDefult, int videoDefault,
                        int underlyDefault) {
        String filePath = fileInfo.thumbnailPath;
        long fileId = fileInfo.dbId;
        String extFromFilename = Utillocal.getExtFromFilename(filePath);
        ...
        if (set)
            ...
            fileImage.setImageResource(fc == FileCategory.Picture ? picDefult
                    : videoDefault);
        ...
        if (!set)
            fileImage.setImageResource(underlyDefault);
    }
