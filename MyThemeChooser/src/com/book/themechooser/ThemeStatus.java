...
import com.book.themechooser.newmechanism.Globals;
...
import static com.book.themes.ThemeManager.THEME_LOCKSCREEN2_PACKAGE;
import static com.book.themes.ThemeManager.STANDALONE;
import static com.book.themes.ThemeManager.THEME_ELEMENTS_PACKAGE;
...
public final class ThemeStatus {
    ...
    private static final String THEME_ROOT_PATH = "/book/theme";
    ...
    private static final String THEME_PACKAGE_PREFIX = "book_LWT_";
    private static final String THEME_WALLPAPER_PREFIX = "book_WP_";
    private static final String THEME_LOCK_WALLPAPER_PREFIX = "book_LWP_";
    ...
    public boolean isWallpaperSettedByPath(String name) {
        ...
        String wallpaper_path = Settings.System.getString(
                mContext.getContentResolver(), "book_wallpaper_path");
...
//                    mAppliedPkgNames[type] = uri;
    ...
    private void checkAppliedTheme() {
        mAppliedPkgNames[THEME_TYPE_STYLE] = SystemProperties.get(
                "persist.sys.themePackageName", "com.book.theme.bookDefaultTheme");
        ...
        if (TextUtils.isEmpty(mAppliedPkgNames[THEME_TYPE_PACKAGE])) {
            ...
            mAppliedPkgNames[THEME_TYPE_PACKAGE] = prefsDownloaded
                    .getString("applied_themePkgName", "com.book.theme.bookDefaultTheme");
            for (int i = THEME_TYPE_PACKAGE + 1; i < THEME_TYPES_COUNT - 1; ++i) {
                mAppliedPkgNames[i] = mAppliedPkgNames[THEME_TYPE_PACKAGE];
            }
            mAppliedPkgNames[THEME_TYPE_LOCK_WALLPAPER] = "com.book.theme.bookDefaultTheme";
    ...
    private void checkDownloadingThemes() {
        Cursor c = null;
        try {
            bookDownloadManager dm = bookDownloadManager
                    .getInstance(mContext.getContentResolver(), mContext.getPackageName());
            bookDownloadManager.Query query = new bookDownloadManager.Query();
            ...
            // + " <> ?"[>  + " AND " + Downloads.Impl.COLUMN_FILE_NAME_HINT + " LIKE '%book/theme%'" <]
    ...
    private ArrayList<Long> checkDownloadingStatus() {
        synchronized (mFailed) {
            ...
            bookDownloadManager dm = bookDownloadManager
                    .getInstance(mContext.getContentResolver(), mContext.getPackageName());
            if (dm instanceof bookDownloadManager) {
